...
export function buildHookBlock(): string {
  return [
    ...
    "  exit 0",
    "fi",
    'custos_stdin_file="${TMPDIR:-/tmp}/custos-pre-push-$$.stdin"',
    'trap \'rm -f "$custos_stdin_file"\' EXIT HUP INT TERM',
    'cat > "$custos_stdin_file"',
    'if ( : < /dev/tty ) 2>/dev/null; then',
    '  CUSTOS_PRE_PUSH_STDIN_FILE="$custos_stdin_file" custos scan --pre-push < /dev/tty',
    "else",
    '  CUSTOS_PRE_PUSH_STDIN_FILE="$custos_stdin_file" custos scan --pre-push',
    "fi",
    "custos_status=$?",
    'rm -f "$custos_stdin_file"',
    "trap - EXIT HUP INT TERM",
    'if [ "$custos_status" -ne 0 ]; then',
    '  exit "$custos_status"',
    "fi",
    hookEnd,
  ].join("\n");
